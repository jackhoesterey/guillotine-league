...
_xw_path = ROOT / "data" / "crosswalk.json"
if _xw_path.exists():
    _by_rank = json.load(open(_xw_path)).get("by_rank") or {}
    for _p in D:
        _rec = _by_rank.get(str(_p["r"])) or {}
        if _rec.get("sleeper_id"):
            _p["sid"] = _rec["sleeper_id"]
...
#stale{display:none;background:rgba(248,81,73,.12);border-bottom:1px solid rgba(248,81,73,.35);
 color:#ffb4ae;padding:8px 18px;font-size:13px}
#stale.on{display:block}
#livepill.live{border-color:var(--floor);color:var(--floor)}
#livepill.stale{border-color:var(--avoid);color:var(--avoid)}
input#draftid{width:140px;background:var(--panel2);border:1px solid var(--line);color:var(--tx);
 padding:4px 8px;border-radius:6px;font-size:12px;outline:none}
input#draftid:focus{border-color:var(--accent)}
...
<div id="stale">UNVERIFIED — stale connection. Picks are not live. Check Sleeper.</div>
<div class="top"><div class="topin">
  ...
  <a class="pill" href="/brief.html" style="text-decoration:none;color:inherit">brief</a>
  <a class="pill" href="/gameday.html" style="text-decoration:none;color:inherit">gameday</a>
  <input id="draftid" placeholder="Sleeper draft id" autocomplete="off">
  <span class="pill" style="cursor:pointer" id="golive">live</span>
...
/* ---------- live Sleeper draft ---------- */
let live = false, liveTimer = null, lastLiveOk = 0, myUserId = null, unmappedN = 0;
const bySid = {};
...
function setLivePill(mode, text){
  ...
  el.textContent = text;
  $("#stale").classList.toggle("on", mode === "stale");
}
...
function applyLivePicks(picks){
  ...
  for (const pk of picks || []){
    const p = bySid[String(pk.player_id)];
    if (!p){ miss++; continue; }
    const mine = myUserId && String(pk.picked_by||"") === String(myUserId);
    next[p.r] = mine ? "mine" : "taken";
  }
  unmappedN = miss;
  st = next;
  save();
...
async function pullLive(){
  ...
  try {
    const r = await fetch("https://api.sleeper.app/v1/draft/"+id+"/picks", {signal: ctrl.signal});
    if (!r.ok) throw new Error("HTTP "+r.status);
    const picks = await r.json();
    lastLiveOk = Date.now();
    applyLivePicks(picks);
    const extra = unmappedN ? ` · ${unmappedN} off-board` : "";
    setLivePill("live", `live · ${picks.length} picks${extra}`);
  } catch (e) {
    if (Date.now() - lastLiveOk > 8000) setLivePill("stale", "UNVERIFIED — stale");
  } finally { clearTimeout(t); }
}

async function startLive(){
  const id = ($("#draftid").value || "").trim();
  if (!id){ alert("Paste a Sleeper draft id (from the draft URL)."); return; }
  live = true;
  lastLiveOk = 0;
  setLivePill("live", "connecting…");
  try {
    const d = await fetch("https://api.sleeper.app/v1/draft/"+id).then(r => r.json());
    const teamsN = (d.settings && d.settings.teams) || teams();
    $("#teams").value = teamsN;
    const order = d.draft_order || {};
    if (myUserId && order[myUserId] != null) $("#slot").value = order[myUserId];
  } catch (e) {}
...
fetch("/players.json")
  ...
  .then(async data => {
    D = data;
    D.forEach(p => { byName[p.r] = p; if (p.sid) bySid[String(p.sid)] = p; });
    load();
    render();
    try {
      const cfg = await fetch("/league.json").then(r => r.ok ? r.json() : null).catch(() => null);
      if (cfg && cfg.draft_id && !$("#draftid").value) $("#draftid").value = cfg.draft_id;
      if (cfg && cfg.username){
        const u = await fetch("https://api.sleeper.app/v1/user/"+encodeURIComponent(cfg.username))
          .then(r => r.ok ? r.json() : null).catch(() => null);
        if (u && u.user_id) myUserId = String(u.user_id);
      }
      if (cfg && cfg.user_id) myUserId = String(cfg.user_id);
      if (cfg && cfg.draft_slot) $("#slot").value = cfg.draft_slot;
    } catch (e) {}
    const q = new URLSearchParams(location.search);
    if (q.get("draft")){ $("#draftid").value = q.get("draft"); startLive(); }
    else if (q.get("live") === "1" && $("#draftid").value) startLive();
  })
